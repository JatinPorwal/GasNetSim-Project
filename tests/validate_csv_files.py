...
# **********************************************************************************************************************
# This script ensures the .csv file conforms to the established format guidelines.
# **********************************************************************************************************************
...
class ValidateCsvFiles:
    ...
    def __init__(self, file_path=None, pipelines_expected_headers=None, pipeline_mandatory_headers=None,
        ...
        """
                Initializes the ValidateCsvFiles class with the path of the CSV file.

                Parameters:
                - file_path (str): Path to the CSV file for validation.

                Defines the expected and mandatory headers for pipelines and nodes CSV files.
        """

        # Defining the expected and mandatory headers for the csv files
        if nodes_expected_headers is None:
    ...
    def validate_pipelines_csv_format(self):
        ...
        with open(self.file_path, 'r') as file:
            ...
            if headers != self.pipeline_expected_headers:
            ...
            for row_num, row in enumerate(reader, start=1):
                missing_values = [key for key in self.pipeline_mandatory_headers
    ...
    def validate_nodes_csv_format(self):
        ...
        with open(self.file_path, 'r') as file:
            ...
            # Check if headers match the expected format
            if headers != self.nodes_expected_headers:
...
if __name__ == "__main__":
    ...
    val = ValidateCsvFiles(file_path=File_path)
    val.validate_pipelines_csv_format()
    val.validate_nodes_csv_format()
